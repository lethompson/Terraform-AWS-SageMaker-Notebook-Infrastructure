resource "aws_s3_bucket" "s3_bucket_1" {
  ...
  tags = {
    Description = "Bucket for storing the Amazon SageMaker deep racer model and training data."
...
resource "aws_s3_bucket_object" "s3_deep_racer_notebook" {
  ...
  etag = "${filemd5("${path.module}/../source/notebooks/DeepRacerLogAnalysis.ipynb")}"
}
